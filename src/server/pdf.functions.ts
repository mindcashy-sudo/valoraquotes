...
interface QuoteItem { name: string; price: number; }
interface QuoteSection { name: string; items: QuoteItem[]; subtotal: number; }
...
function drawFooter(ctx: Ctx, pageNum: number, pageTotal: number) {
  ...
  const left = [studio.studio_name, studio.vat_number ? `P.IVA ${studio.vat_number}` : null]
    .filter(Boolean).join("   ·   ");
...
async function drawHeader(
  ...
  if (logoBytes && logoMime) {
    try {
      ...
      if (img) {
        ...
        let h = LOGO_BOX_H;
        let w = h * ratio;
        if (w > LOGO_BOX_W) { w = LOGO_BOX_W; h = w / ratio; }
  ...
  // Right column: studio meta
  let yR = top - 4;
  const lineH = (s: number) => s + 4;
  const fit = (text: string, size: number, f: PDFFont) => {
    let s = sanitize(text);
    if (f.widthOfTextAtSize(s, size) <= rightColW) return s;
    while (s.length > 4 && f.widthOfTextAtSize(s + "...", size) > rightColW) s = s.slice(0, -1);
    return s + "...";
  };
  const drawR = (text: string | null | undefined, size: number, f: PDFFont, color = MUTED) => {
    if (!text) return;
    dTextRight(ctx, fit(text, size, f), PAGE_W - MX, yR, size, f, color);
    yR -= lineH(size);
  };
  if (logoDrawn && studio.studio_name) drawR(studio.studio_name, 9, bold, INK);
...
function drawExecutiveSummary(ctx: Ctx, q: QuoteContent) {
  const { font, bold } = ctx;
  ensure(ctx, 160);
...
function drawAcceptance(ctx: Ctx) {
  const { font, bold, studio } = ctx;
  ensure(ctx, 170);
  ctx.y -= 6;
...
export const generateQuotePdf = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (studio.logo_url) {
      try {
        const { data: file } = await supabase.storage.from("studio-assets").download(studio.logo_url);
        if (file) {
    ...
    const quoteNumber =
      data.quoteNumber ??
      `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, "0")}${String(now.getDate()).padStart(2, "0")}-${String(Math.floor(Math.random() * 1000)).padStart(3, "0")}`;
